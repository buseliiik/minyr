...
// TestCountLines tester om CountLines funksjonen returnerer riktig antall linjer i filen.
...
// TestConvertLines tester om ProcessLine funksjonen konverterer riktig fra gammel format til nytt format.
func TestConvertLines(t *testing.T) {
	...
	tests := []test{
		...
		{input: "Data er gyldig per 18.03.2023 (CC BY 4.0), Meteorologisk institutt (MET);;;",
			want: "Data er basert på gyldig data (per 18.03.2023) (CC BY 4.0) fra Meteorologisk institutt (MET);endringen er gjort av Buse"},
	}
...
// TestGetAverageTemperature tester om GetAverageTemperature funksjonen returnerer riktig gjennomsnittstemperatur fra en fil.
